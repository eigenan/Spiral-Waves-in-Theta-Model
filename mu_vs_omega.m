...
r_min=0.1;                          r_max=20;
p_min=0;                            p_max=2*pi;
r_grid_size=360;                    p_grid_size=120;
...
ds=5;                               increment_init=1e-3;
mu_tol=1e-6;                        overshoot=0.2;
few_newton=1.1;                     newton_fail=0.8;
...
mu_list(end+1)=nu0(end,1);
omega_list(end+1)=nu0(end-1,1);
mu_list(end+1)=nu1(end,1);
omega_list(end+1)=nu1(end-1,1);
...
save("dispersion_data.mat","omega_list","mu_list");

% figure(3)
% plot(mu_list,omega_list,'b.-')
